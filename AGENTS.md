...
The neural anatomy explorer is read-only. Use annotated classes and actual graph edges; keep experimental input/readout choices distinct from biological sensory/motor categories. Label landmark subsampling and missing coordinates. Do not draw fabricated positions or represent soma-to-soma lines as neuron arbors. Biological L1 uses graded potentials; the existing generic spiking model is not validated L1 physiology. The standalone explorer normally uses gx10-b/8768 and must not interrupt ongoing experiments.


The separate `fruitflybrain.fullscale` runner tests the complete retained graph
with bounded recurrent rate dynamics, engineered 24-feature sensory input and an
all-neuron action readout. It is not the live Exp 2 service or validated fly
physiology. Exp 2 v0.4 was prospective; v0.5 reports the completed five-seed test.
Q-learning outperformed this model; internal changes were tiny. The zero-recurrence
CPU diagnostic is explicitly post hoc and must never replace a claimed full-graph
run. Preserve raw runs/source archives and separate primary from diagnostic results.
Any stronger learning follow-up needs a dated amendment, training/validation/test
separation and fresh evaluation conditions; do not retune on these reported tests
and present the reused outcomes as an independent comparison.
