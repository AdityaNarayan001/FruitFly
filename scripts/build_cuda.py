...
cmd=[compiler,'-O3','--fmad=false','-std=c++17','-arch='+arch,'-shared','-Xcompiler=-fPIC',str(root/'native/lif.cu'),'-o',str(root/'build/libffb.so')]
subprocess.run(cmd,check=True)
meta={'architecture':arch,'command':cmd,'compiler':subprocess.check_output([compiler,'--version'],text=True),'sha256':hashlib.sha256((root/'build/libffb.so').read_bytes()).hexdigest()}
(root/'build/build.json').write_text(json.dumps(meta,indent=2)+'\n')
